...
<template>
  <div class="box">
    <span class="text-lg font-bold">{{ post.body }}</span>
    <h1 class="title">comments ({{ post.amountcmt }})</h1>
    <CommentItem v-for="comment in post.comments" :comment="comment" :key="comment.id" />
    <button class="bg-green-500 hover:bg-green-600 text-white font-bold rounded p-2"
      @click="() => (isComment = !isComment)">
      Add Comment
    </button>
    <NewComment v-if="isComment" :postId="post.id" @addComment="addNewComment" />
  </div>
</template>
